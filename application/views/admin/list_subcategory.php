			                  		...
			                  		<div class="fixed-table-toolbar">
			                  			<div class="bs-bars pull-left">
			                  				<div class="btn-group hidden-sm-down" id="exampleToolbar" role="group">
							                    <button type="button" class="btn btn-success btn-outline btn-sm" onclick="window.location.href = '<?=base_url('Admin/formSubcategory');?>';">
			                  		...
			                  		<div class="" style="padding-bottom: 0px;">
			                  			<div class="fixed-table-body">
			                  				<table id="exampleTableToolbar" data-mobile-responsive="true" class="table table-hover" data-pagination="true" data-search="true" style="margin-top: 0px;">
			                    				<thead style="">
			                    					<tr>
			                    						...
			                    						<th style="" data-field="name">
			                    							<div class="th-inner ">SubCategory Name</div>
			                    							<div class="fht-cell"></div>
			                    						</th>
                                                        <th style="" data-field="categoryname">
			                    				...
			                    				<?php if(!empty($cates_list)) : ?>
			                  					<tbody>
			                  						<?php 
													$i = 0;
													foreach($cates_list as $cates_detail) : 
													?>
			                  						<tr> 
			                  							<td class=""><?=$i+1; ?></td> 
			                  							<td style=""><?=$cates_detail['subcate_name']; ?></td>
                                                        <td style=""><?=$cates_detail['cate_id']; ?></td>
			                  							<td style=""><?=date("d/m/Y ", strtotime($cates_detail['create_date']) ); ?></td>
			                  							<td>
			                  								<button type="button" class="btn btn-round btn-warning btn-sm" onclick="window.location.href = '<?=base_url('Admin/edit_subcategory/'.$cates_detail['sub_id']);?>';"><i class="icon wb-pencil" aria-hidden="true"></i></button>
			                  								<button type="button" class="btn btn-round btn-danger btn-sm" 
															  onclick="deleSub(<?=$cates_detail['sub_id'];?>)"><i class="icon wb-close" aria-hidden="true"></i></button>
			                  							</td> 
			                  						</tr>
			                  						<?php
			                  							$i++;
...
<div class="modal fade bs-example-modal-sm" id="modaldelete" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog modal-sm">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-success" onclick="deleteSubcate()"><i class="fa fa-check"></i> Confirm</button>
	...
	<script type="text/javascript">
		function deleSub(subcateid)
		{
			$('#subcate_id').val(subcateid);
		...
		function deleteSubcate()
		{
			$.ajax({
				type 	: 'POST',
				url 	: '<?=base_url("Admin/delSubcate"); ?>',
				data 	: {id:$('#subcate_id').val(), action:'delSubcate', '<?=$this->security->get_csrf_token_name(); ?>':'<?=$this->security->get_csrf_hash(); ?>'},
