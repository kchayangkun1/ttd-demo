...
<head>
	...
	<title>รายการศัลยกรรม & สกิน | Lenista Clinic </title>
...
<body class="">
		...
		<div class="page-content container-fluid">
	      	...
	      	<div class="panel">
		        <div class="panel-heading">
		          	<h3 class="panel-title">รายการศัลยกรรม & สกิน</h3>
		        </div>
			                  		...
			                  		<div class="fixed-table-toolbar">
			                  			<div class="bs-bars pull-left">
			                  				<div class="btn-group hidden-sm-down" id="exampleToolbar" role="group">
							                    <button type="button" class="btn btn-success btn-outline btn-sm" onclick="window.location.href = '<?=base_url('Admin/modelForm');?>';">
							                      	<i class="icon wb-plus" aria-hidden="true"></i> Add Model
							                    </button>
			                  				</div>
			                  			</div>
			                  		</div>
			                  					...
			                  					<tbody>
													...
													/*
													  [id] => 1
														[brand_id] => 1
														[car_id] => 1
														[name] => SR C package (Hybrid)
														[is_active] => 1
														[create_date] => 2019-07-31 11:18:44
														[update_date] => 2019-08-20 09:55:23
														[car_name] => ศัลยกรรมจมูก
														[brand_name] => ศัลยกรรม
													*/
													foreach($car_lists as $model_dsc) : 
													?>
			                  						<tr> 
			                  							<td class=""><?=$i+1; ?></td>
			                  							<td style=""><?=$model_dsc['name']; ?></td>
														  <td style=""><?=$model_dsc['brand_name']; ?></td>
                                                        <td style=""><?=$model_dsc['car_name']; ?></td>
			                  							<td style=""><?=date("d/m/Y H:i:s", strtotime($model_dsc['create_date'])); ?></td>
			                  							<td>
			                  								<button type="button" class="btn btn-round btn-warning btn-sm" onclick="window.location.href = '<?=base_url('Admin/modelEdit/'.$model_dsc['id']);?>';"><i class="icon wb-pencil" aria-hidden="true"></i></button>
			                  								<button type="button" class="btn btn-round btn-danger btn-sm" onclick="delS(<?=$model_dsc['id']; ?>, <?=$model_dsc['is_active']; ?>)"><i class="icon wb-close" aria-hidden="true"></i></button>
			                  							</td> 
			                  						</tr>
			                  						<?php
			                  							$i++;
		...
		<div class="modal-dialog modal-sm">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<!--  -->
					<button type="button" class="btn btn-success" onclick="deleteModel()"><i class="fa fa-check"></i> Confirm</button>
					<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
				</div>
			</div>
		</div>
    ...
    function changeSt()
    {
   		$.ajax({
		  	type 	: 'POST',
		  	url 	: 'funcQuery.php',
		  	data 	: {sid:$('#model_id').val(), st:$('#sStatus').val(), action:'changeStse'},
     ...
     function deleteModel()
    {
   		$.ajax({
		  	type 	: 'POST',
		  	url 	: '<?=base_url("Admin/distroyModel"); ?>',
		  	data 	: {sid:$('#s_id').val(),st:$('#s_status').val(), action:'delModel', '<?=$this->security->get_csrf_token_name(); ?>':'<?=$this->security->get_csrf_hash(); ?>'},
		  	success: function(data) {
				if (data == 'true') {
