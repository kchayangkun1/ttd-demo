...
<head>
	...
	<title>Edit Product | glovepfs </title>
...
<body class="">
	...
	<?php 
		...
		// multiple image
		$FileUploader=$this->upload_product_model->fileUploaded($product_detail[0]['id']);
			            ...
			            <div class="panel-body container-fluid">
                              ...
                              <input type="hidden" name="<?=$this->security->get_csrf_token_name();?>" 
                                ...
                                <div class="form-group form-material" data-plugin="formMaterial">
				                  	...
				                  	<input type="text" class="form-control" id="name" name="name" value="<?=$product_detail[0]['name'];?>" placeholder="Name" required>
				                </div>
                                <div class="form-group form-material" data-plugin="formMaterial">
				                  	<label class="form-control-label" for="price">Price</label>
				                  	<input type="text" class="form-control" id="price" name="price" value="<?=$product_detail[0]['price'];?>" placeholder="เช่น 1200" onkeypress='return event.charCode >= 48 && event.charCode <= 57' required>
				                </div>
								<div class="form-group form-material" data-plugin="formMaterial">
				                  	<label class="form-control-label" for="xs">Size xs</label>
				                  	<input type="text" class="form-control" id="size_xs" name="size_xs" value="<?=$product_detail[0]['size_xs'];?>" placeholder="กรุณาพิมพ์ไซส์ เช่น xs">
				                </div>
								<div class="form-group form-material" data-plugin="formMaterial">
				                  	<label class="form-control-label" for="s">Size s</label>
				                  	<input type="text" class="form-control" id="size_s" name="size_s" value="<?=$product_detail[0]['size_s'];?>" placeholder="กรุณาพิมพ์ไซส์ เช่น s">
				                </div>
								<div class="form-group form-material" data-plugin="formMaterial">
				                  	<label class="form-control-label" for="m">Size m</label>
				                  	<input type="text" class="form-control" id="size_m" name="size_m" value="<?=$product_detail[0]['size_m'];?>" placeholder="กรุณาพิมพ์ไซส์ เช่น m">
				                </div>
								<div class="form-group form-material" data-plugin="formMaterial">
				                  	<label class="form-control-label" for="l">Size l</label>
				                  	<input type="text" class="form-control" id="size_l" name="size_l" value="<?=$product_detail[0]['size_l'];?>" placeholder="กรุณาพิมพ์ไซส์ เช่น l">
				                </div>
								<div class="form-group form-material" data-plugin="formMaterial">
				                  	<label class="form-control-label" for="xl">Size xl</label>
				                  	<input type="text" class="form-control" id="size_xl" name="size_xl" value="<?=$product_detail[0]['size_xl'];?>" placeholder="กรุณาพิมพ์ไซส์ เช่น xl">
				                </div>
										...
										<select class="form-control" name="sub_cate" required>
											...
											<optgroup label="<?=$cate_val['c_name'];?>">                         
												...
												<?php foreach ( $subcate_list as $subcate_val) : 
                                                        if($product_detail[0]['cat_name'] == $subcate_val['sub_id']) : 
                                                            $selected='selected';
                                                        else :
                                                            $selected='';
                                                        endif;
                                                    ?>
												<option value="<?=$subcate_val['sub_id'];?>" <?=$selected;?>><?=$subcate_val['subcate_name'];?></option>
												<?php endforeach; ?>
											</optgroup>
											<?php endforeach; ?>
										</select>
								...
								<div class="form-group form-material form-material-file" data-plugin="formMaterial">
									...
									<span class="badge badge-pill badge-warning">ชื่อไฟล์เป็นภาษาอังกฤษหรือตัวเลขเท่านั้น!</span>
			                      	<input type="file" id="covImg" name="covImg" data-plugin="dropify" data-default-file="<?=base_url('./assets/images/product/cover/'.$product_detail[0]['id'].'/'.$product_detail[0]['img_cover']);?>" data-allowed-file-extensions="png jpg jpeg PNG JPG JPEG"/>
