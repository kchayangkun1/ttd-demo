...
<head>
	...
	<title>หมวดหมู่ | Lenista Clinic</title>
...
<body class="">
			                  			...
			                  			<div class="bs-bars pull-left">
			                  				<!-- <div class="btn-group hidden-sm-down" id="exampleToolbar" role="group">
							                    <button type="button" class="btn btn-success btn-outline btn-sm" onclick="window.location.href = '<?//=base_url('Admin/FormCategory');?>';">
							                      	<i class="icon wb-plus" aria-hidden="true"></i>เพิ่ม หมวดหมู่
							                    </button>
			                  				</div> -->
			                  			</div>
			                  		</div>
			                    				...
			                    				<thead style="">
			                    					<tr>
			                    						...
			                    						<th style="" data-field="name">
			                    							<div class="th-inner ">ชื่อหมวดหมู่</div>
			                    							<div class="fht-cell"></div>
			                    						</th>
			                    						<th style="" data-field="">
			                    				...
			                    				<?php if(!empty($categories)) : ?>
			                  					<tbody>
													...
													?>
			                  						<tr> 
			                  							<td class=""><?=$i+1; ?></td> 
			                  							<td style=""><?=$cate_val['name']; ?></td>
			                  							<td style=""><?=date("d/m/Y H:i:s", strtotime($cate_val['create_date'])); ?></td>
			                  							<td>
			                  								<button type="button" class="btn btn-round btn-warning btn-sm" onclick="window.location.href = '<?=base_url('Admin/EditeReviewCategory/'.$cate_val['id']);?>';"  data-toggle="tooltip" data-placement="top" title="แก้ไข"><i class="icon wb-pencil" aria-hidden="true"></i></button>
			                  								<button type="button" class="btn btn-round btn-danger btn-sm" onclick="delS(<?=$cate_val['id']; ?>, <?=$cate_val['is_active']; ?>)" data-toggle="tooltip" data-placement="top" title="ลบ"><i class="icon wb-close" aria-hidden="true"></i></button>
			                  							</td> 
			                  						</tr>
			                  						<?php
			                  							$i++;
		...
		<div class="modal-dialog modal-sm">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<!--  -->
					<button type="button" class="btn btn-success" onclick="deleteBrand()"><i class="fa fa-check"></i> Confirm</button>
					<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
				</div>
			</div>
		</div>
    ...
    function changeSt()
    {
   		$.ajax({
		  	type 	: 'POST',
		  	url 	: 'funcQuery.php',
		  	data 	: {sid:$('#brand_id').val(), st:$('#sStatus').val(), action:'changeStse'},
     ...
     function deleteBrand()
    {
   		$.ajax({
		  	type 	: 'POST',
		  	url 	: '<?=base_url("Admin/distroyBrand"); ?>',
		  	data 	: {sid:$('#s_id').val(),st:$('#s_status').val(), action:'delBrand', '<?=$this->security->get_csrf_token_name(); ?>':'<?=$this->security->get_csrf_hash(); ?>'},
		  	success: function(data) {
				if (data == 'true') {
