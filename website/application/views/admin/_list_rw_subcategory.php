...
<body class="">
			                  		...
			                  		<div class="fixed-table-toolbar">
			                  			<div class="bs-bars pull-left">
			                  				<div class="btn-group hidden-sm-down" id="exampleToolbar" role="group">
							                    <button type="button" class="btn btn-success btn-outline btn-sm" onclick="window.location.href = '<?=base_url('Admin/formRwSubcategory');?>';">
							                      	<i class="icon wb-plus" aria-hidden="true"></i> เพิ่ม หมวดหมู่ย่อย
							                    </button>
			                  				</div>
			                  			</div>
			                  		</div>
			                  				...
			                  				<table id="exampleTableToolbar" data-mobile-responsive="true" class="table table-hover" data-pagination="true" data-search="true" style="margin-top: 0px;">
			                    				<thead style="">
			                    					<tr>
			                    						...
			                    						<th style="" data-field="sub-name">
			                    							<div class="th-inner ">หมวดหมู่ย่อย</div>
			                    							<div class="fht-cell"></div>
			                    						</th>
                                                        <th style="" data-field="category-name">
			                    							<div class="th-inner ">หมวดหมู่หลัก</div>
			                    							<div class="fht-cell"></div>
			                    						</th>
			                    						<th style="" data-field="">
			                    				...
			                    				<?php if(!empty($rw_subcategories)) : ?>
			                  					<tbody>
			                  						<?php 
													$i = 0;
													foreach($rw_subcategories as $rwsubcate_val) : 
													?>
			                  						<tr> 
			                  							<td class=""><?=$i+1; ?></td>
			                  							<td style=""><?=$rwsubcate_val['sub_name']; ?></td>
                                                        <td style=""><?=$rwsubcate_val['cate_rw_name']; ?></td>
			                  							<td style=""><?=date("d/m/Y H:i:s", strtotime($rwsubcate_val['update_date'])); ?></td>
			                  							<td>
			                  								<button type="button" class="btn btn-round btn-warning btn-sm" onclick="window.location.href = '<?=base_url('Admin/editRwSubcategory/'.$rwsubcate_val['id']);?>';"><i class="icon wb-pencil" aria-hidden="true"></i></button>
			                  								<button type="button" class="btn btn-round btn-danger btn-sm" onclick="delS(<?=$rwsubcate_val['id']; ?>, <?=$rwsubcate_val['is_active']; ?>)"><i class="icon wb-close" aria-hidden="true"></i></button>
			                  							</td> 
			                  						</tr>
			                  						<?php
			                  							$i++;
		...
		<div class="modal-dialog modal-sm">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-success" onclick="deleteCar()"><i class="fa fa-check"></i> Confirm</button>
					<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
				</div>
			</div>
		</div>
    ...
    function changeSt()
    {
   		$.ajax({
		  	type 	: 'POST',
		  	url 	: 'funcQuery.php',
		  	data 	: {sid:$('#car_id').val(), st:$('#sStatus').val(), action:'changeStse'},
     ...
     function deleteCar()
    {
   		$.ajax({
		  	type 	: 'POST',
		  	url 	: '<?=base_url("Admin/distroyCar"); ?>',
		  	data 	: {sid:$('#s_id').val(),st:$('#s_status').val(), action:'delCar', '<?=$this->security->get_csrf_token_name(); ?>':'<?=$this->security->get_csrf_hash(); ?>'},
		  	success: function(data) {
				if (data == 'true') {
