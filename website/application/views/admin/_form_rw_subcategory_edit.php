					...
					<!-- Panel Static Labels -->
		          	<div class="panel">
			            ...
			            <div class="panel-body container-fluid">
			              	<form action="<?=base_url('Admin/rwSubcategoryUpdate');?>" id="serviceAdd" name="serviceAdd" class="form-horizontal" method="post" enctype="multipart/form-data">
                                ...
                                <div class="form-group form-material">
									...
									<select class="form-control" name="rw_category" required>
                                        ...
                                        <?php foreach ($rw_categories as $rwcate_val) : ?>
                                            <option value="<?=$rwcate_val['id']; ?>" <?php if($rw_subcate[0]['category_review_id'] == $rwcate_val['id']){ echo 'selected'; } ?>><?=$rwcate_val['name']; ?></option>
                                        <?php endforeach ?>
									</select>
								</div>
				                <div class="form-group form-material" data-plugin="formMaterial">
				                  	<label class="form-control-label" for="title">ชื่อหมวดหมู่ย่อย</label>
				                  	<input type="text" class="form-control" id="rw_subcate" name="rw_subcate" value="<?=$rw_subcate[0]['name']?>" placeholder="เช่น รีวิวเสริมจมูก" required>
				                </div>
				                <div class="text-right">
                                    <input type="hidden" name="subcate_rw_id" id="subcate_rw_id" value="<?=$rw_subcate[0]['id']?>">
